...
import com.bylh.yyb.follow.net.tcp.socket.CharsetUtil;
import com.bylh.yyb.follow.net.tcp.socket.ExceptionThrower;
import com.bylh.yyb.follow.net.tcp.socket.HeartBeatHelper;
import com.bylh.yyb.follow.net.tcp.socket.SocketClientAddress;
import com.bylh.yyb.follow.net.tcp.socket.SocketClientDelegate;
import com.bylh.yyb.follow.net.tcp.socket.SocketClientReceiveDelegate;
import com.bylh.yyb.follow.net.tcp.socket.SocketClientSendingDelegate;
import com.bylh.yyb.follow.net.tcp.socket.SocketConfigure;
import com.bylh.yyb.follow.net.tcp.socket.SocketInputReader;
import com.bylh.yyb.follow.net.tcp.socket.SocketPacket;
import com.bylh.yyb.follow.net.tcp.socket.SocketPacketHelper;
import com.bylh.yyb.follow.net.tcp.socket.SocketResponsePacket;
import com.bylh.yyb.follow.utils.ClientBaseUtils;
...
public class SocketClient {
    ...
    public static  byte[] SendIOCPPak(byte[] sendBuf,int nLen) {
        byte[] buf = new byte[nLen + 16];
        int len = nLen;
        len = len & 0x007FFFFF;
        int Flag = 1 << 23;
        Flag = Flag | 0x01000000;
        Flag += len;
        ClientBaseUtils.toLH(buf, 0, Flag);

        int Offset = 0;
        ClientBaseUtils.toLH(buf, 4, Offset);

        int Identify = 1;
        ClientBaseUtils.toLH(buf, 8, Identify);

        int CheckSum = Flag + Offset + Identify;
        ClientBaseUtils.toLH(buf, 12, CheckSum);

        if (sendBuf != null) {
            System.arraycopy(sendBuf, 0, buf, 16, nLen);//将pBuf装进前20-nLength位
        }
        return buf;
    }
    ...
    protected SocketClientAddress address;
...
//                e.printStackTrace();
    ...
    public SocketInputReader inputReader = null;
